...
def standardize_date(df: pd.DataFrame, columns=None, output_format="%Y-%m-%d") -> pd.DataFrame:
    """
    Convert semua kolom tanggal ke format standar ISO
    - columns: list nama kolom, kalau None coba semua kolom
    - output_format: strftime format
    """
    def parse_date(val):
        if pd.isna(val):
            return val
    ...
    for col in columns:
        # deteksi kolom tanggal: sederhana pakai kata "date" atau object
